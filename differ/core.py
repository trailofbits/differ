...
class TraceTemplate:
    ...
    def __post_init__(self):
        ...
        self._concurrent_template = None
    ...
    def setup_template(self) -> Optional[jinja2.Template]:
        ...
        if not self._setup_template and self.setup:
            if self.script_exit_on_first_error:
                ...
                content = self.setup
            self._setup_template = JINJA_ENVIRONMENT.from_string(content)
    ...
    def teardown_template(self) -> Optional[jinja2.Template]:
        ...
        if not self._teardown_template and self.teardown:
            ...
            self._teardown_template = JINJA_ENVIRONMENT.from_string(content)
    ...
    def concurrent_template(self) -> Optional[jinja2.Template]:
        ...
        """
        if not self._concurrent_template and self.concurrent:
            if self.script_exit_on_first_error:
                content = f'set -e\n\n{self.concurrent.run}'
            else:
                content = self.concurrent.run
            self._concurrent_template = JINJA_ENVIRONMENT.from_string(content)
        return self._concurrent_template
